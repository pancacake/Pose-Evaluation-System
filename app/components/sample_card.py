...
class SampleCard_small(CardWidget):
    """ Sample card """

    def __init__(self, routeKey, index, parent=None):
        super().__init__(parent=parent)
        self.index = index
        self.routekey = routeKey

        self.pushButton = PushButton(FIF.FOLDER,"See your Pose Reconstruction")
        ...
        self.hBoxLayout.addLayout(self.vBoxLayout)
        self.vBoxLayout.addStretch(1)
        self.pushButton.clicked.connect(self.small_click)
...
class SampleCardView_small(QWidget):
    """ Sample card view """
    ...
    def addSampleCard_small(self,  routeKey, index):
        """ add sample card """
        card = SampleCard_small(routeKey, index, self)
        self.flowLayout.addWidget(card)